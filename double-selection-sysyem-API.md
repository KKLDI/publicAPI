...
```
{
	status: true
	msg: ""
	code:"200：登录成功，并且返回用户信息，601：登录失败，原因是未绑定学号或工号"
	data:"登录成功用户信息，若登录失败则返回null"
}
```
...
```
{
	status: true
	msg: ""
	code:"绑定成功，并且返回用户信息，602：绑定失败，原因是没有该学号或工号 603：绑定失败，原因是该学号或者工号已经被绑定"
	data:"绑定成功用户信息，若绑定失败则返回null"
}
```
### 2.1.3 用户解除绑定微信接口
...
```
{
	status: true
	msg: ""
	code:"解除绑定学号或者工号接口,200：解除成功 604：解除失败,没有次openID 非法登录"
	data:"null"
}
```